...
#property description "Owned EA repair: one evaluation per bar; no orders"
...
void Check(const string id, const bool actual, const bool expected)
{
   ...
   {
      g_passed++;
      PrintFormat("R49_AFTER PASS %s", id);
   }
   else
   {
      g_failed++;
      PrintFormat("R49_AFTER FAIL %s expected=%s actual=%s", id,
...
int OnInit()
{
   ...
   PrintFormat("R49_AFTER SUMMARY passed=%d failed=%d", g_passed, g_failed);
