...
void OnStart()
{
   ...
   const string summary = StringFormat("R49_AFTER SUMMARY passed=%d failed=%d", g_passed, g_failed);
   Print(summary);
   Comment("R49 AFTER | HolaPrime MT5 | synthetic gate cases\n", g_lines,
           summary, "\nRead-only. No orders or customer data.");
   ChartRedraw(0);
   Sleep(1500);
   const bool saved = ChartScreenShot(0, "SCA_R49_HolaPrime_After_20260924.png", 1280, 720, ALIGN_LEFT);
   PrintFormat("R49_AFTER screenshot_saved=%s", saved ? "true" : "false");
   Sleep(45000);
   Comment("");
}
